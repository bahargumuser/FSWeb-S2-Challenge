...
/* ÖRNEK GÖREV:
	cumleKur fonksiyonuna aşağıdakileri uygulayın:
		...
		Aynı fonksiyonu foo(); şeklinde de çağırabiliriz.
...
//Bu dizinin içinde en çok 5 en az 1 adet string bulunan diziler bulunmaktadır. 
//Aşağıdaki görevlerde aksi belirtilmedikçe bu dizi kullanılacaktır.
...
function cumlelereDonustur(cumleler, ayrac=",") {
	...
	cumleler.map((diziler=> {

		let yeniCumler = diziler.join(ayrac) 

		newSentence.push(yeniCumler)
...
/* GÖREV 2:
		paragrafOlustur fonksiyonuna aşağıdakileri uygulayın.
			...
			5. Oluşturulan bu dizinin tek sayılı anahtarlarını(index) [1,3,5,7,9] kullanarak 
			ilk 5 elemanı geriçağırım olarak çağırılan cumleKur dizisinin parametreleri olarak aktarılarak çağırılacak
			ÖRNEK: callback(dizi[1],dizi[2],dizi[3],dizi[5],dizi[7])
			6. Oluşturulan paragraf döndürülecek
	*/
	
function paragrafOlustur(cumleler, cumleKur, cumlelereDonustur){
	
        const dizi = cumlelereDonustur(cumleler , ' ') //yukarıdaki fonksiyonu çağırdım, dizi=cümlelerden oluşturduğum yeni dizinin adı
        let paragraf = cumleKur(dizi[1], dizi[3], dizi[5], dizi[7] , dizi[9])

        return paragraf
      }
      
 // let result =  paragrafOlustur(cumleler, cumlelereDonustur, cumleKur)
...
/* 			3b.  Bir tavşan ve bir kirpi arkadaşlar sebzeler dizimizin peşine düştü. Tavşan => 🐇 , 
Kirpi=> 🦔 , Tavşanla kirpi sebzeleri ele geçirmek için bir plan kurdular.
Tavşan diziye önden saldıracak, kirpi ise arkalarından dolaşacak.
Varsayalım ki arkadaşların planları başarılı oldu.
...
/* 			3c. manav isminde bir dizi oluşturun.`meyveler` dizisi ilk elemanlara,
`sebzeler` dizisi son elemanlara denk gelecek şekilde,
...
/* 	GÖREV 4:
		...
		Bunun için emojiler adında bir nesne tanımlamışlar.
		Kullanıcının gönderdiği mesaj stringi içinde emojiler nesnesinin anahtarı(index) bulunuyorsa,
		...
		Burdan yola çıkarak emojileriDonustur fonksiyonuna aşağıdakileri uygulayın.
			...
			3. mesaj stringi emoji nesnesinde tanımlı olan tüm sembolleri istenilen değere dönüştürecek (.replaceAll metodu kullanılacak)
				...
				yani hem :d hem de :D sembolleri 😁'a dönüşmelidir. bunun için (.toUpperCase ve .toLowerCase metotlarından istediğinizi
					kullanabilirsiniz.)
